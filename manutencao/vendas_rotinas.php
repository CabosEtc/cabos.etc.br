...
if($num_rows>0 ){ // Numero está cadastrado no sistema(será dado novo numero).
...
// Inicio da montagem do texto da mensagem
	...
	while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
		...
		$msg=$msg."Este pedido é referente a nota".$nrnota_inteiro."	</body>
		</html>";
...
if($flag_envio_email=="sim"){
	...
	{ // se o email pode ser enviado
			echo "Sua nota possui itens relacionados na lista de pedidos automático, sua mensagem foi enviada para ".$emaildestino." com sucesso.<p>";
...
// ------------------------------------------------------------------------------------
	...
	$query="INSERT INTO notas(idnota, cdloja, nrnota, dtnota, vlnota, desconto, garantia, formapagamento, idvendedor, hrnota) VALUES (null, $cdloja, $nrnota_inteiro, '$dtnota_eua', $vlnota_ponto, $desconto_ponto, $garantia, '$idformapagamento','$idusuario','$hrnota')";
	$resultado = mysql_query($query,$conexao);
	...
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant1, '$cdproduto1', $vlunitario1)";
	$resultado = mysql_query($query,$conexao);
	//echo $query;
	
	if ($quant2<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant2, '$cdproduto2', $vlunitario2)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant3<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant3, '$cdproduto3', $vlunitario3)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant4<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant4, '$cdproduto4', $vlunitario4)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant5<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant5, '$cdproduto5', $vlunitario5)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant6<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant6, '$cdproduto6', $vlunitario6)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant7<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant7, '$cdproduto7', $vlunitario7)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant8<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant8, '$cdproduto8', $vlunitario8)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant9<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant9, '$cdproduto9', $vlunitario9)";
	$resultado = mysql_query($query,$conexao);
	}

	if ($quant10<>""){
	$query="INSERT INTO notas_detalhes (iddetalhe, idnota, quantidade, cdproduto, vlproduto) VALUES (null, $idnota, $quant10, '$cdproduto10', $vlunitario10)";
	$resultado = mysql_query($query,$conexao);
	}
	...
	if($num_rows>0){
		echo "<font color='#FF0000'>Atencao: O numero da nota foi alterado para ".$nrnota_inteiro.", porque alguem utilizou este numero de nota antes de voce finalizar a operacao, por favor verifique antes de imprimir</font>";
		}
}
...
?>
    ...
    <a href="../manutencao/vendas.php">Inserir</a> nova nota
    </p>
    
    <p style="padding-top:40px; padding-left:750px">
    <a href="../manutencao/vendas.php?user=14">Inserir</a> nova nota Cabos 2
    </p>
