...
echo "<h3>Relatório para conferência de estoque (oficial) - ".$nomeloja."</h3><br>";
...
while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
	...
	echo "<tr><td width='350'>Nome do Produto</td><td width='30'>Código</td><td align='right' width='100'>Entradas</td><td align='left' style='padding-left:20px' width='150'>Vendidos</td><td align='left' style='padding-left:20px' width='150'>Estoque atual</td></tr>";
	...
	while ($sub_row = mysql_fetch_array($sub_resultado, MYSQL_NUM)) {
		...
		$sub_query3="SELECT sum(quantidade) as entrada from estoque_oficial WHERE cdproduto='".$cdproduto."' and (historico>'50') and cdloja='".$cdloja."'"; // codigo maiores que 50 sao de entrada
		//echo $sub_query;
		$sub_resultado3 = mysql_query($sub_query3,$conexao);
		$entrada_estoque=mysql_result($sub_resultado3,0,0);
		
		$sub_query4="SELECT sum(notas_oficial_detalhes.quantidade) as vendidos from notas_oficial_detalhes, notas_oficial WHERE notas_oficial_detalhes.idnota=notas_oficial.idnota AND notas_oficial_detalhes.cdproduto='".$cdproduto."' and notas_oficial.cdloja='".$cdloja."'";
		//echo $sub_query4;
		$sub_resultado4 = mysql_query($sub_query4,$conexao);
		$vendidos=mysql_result($sub_resultado4,0,0);

		$sub_query5="SELECT sum(quantidade) as perdas from estoque_oficial WHERE cdproduto='".$cdproduto."' and (historico=2) and cdloja='".$cdloja."'"; // historico 2 = perda
		//echo $sub_query;
		...
		if ($entrada_estoque<>"" | $vendidos<>""){
			echo "<tr><td>".$nome."</td><td>".$cdproduto."</td><td align='right'>".$entrada_estoque."</td><td align='left' style='padding-left:20px'>".$vendidos."</td><td align='left' style='padding-left:20px'>".$estoque."</td></tr>";
		}
