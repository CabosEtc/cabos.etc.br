...
//echo $query;
echo "<table>";
echo "<tr><td>Dia</td><td>Hora</td><td>Loja</td><td>&nbsp</td><td align='center'>Id</td><td>Marca</td><td>Produto/Localizador</td><td>&nbsp</td><td>Loja$</td><td>Cabo$</td><td>Flag</td><td>Cabo$2</td><td>Flag</td><td align='center'>BD</td><td align='center'>Link Grupo</td><td align='center'>Link ID</td></tr>";
...
while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
 ...
 $query_nosso_preco_cb2="SELECT links_boadica_detalhes_snapshot.preco FROM links_boadica_detalhes_snapshot WHERE id_loja='451' AND id_produto='$id_produto'";
  ...
  $query_detalhes="SELECT links_boadica_detalhes_snapshot.data,links_boadica_detalhes_snapshot.preco, links_boadica.produto, lojas_boadica.nome, lojas_boadica.flag_predio, links_boadica.link, links_boadica.marca, links_boadica.cdproduto, links_boadica.descricao, links_boadica_detalhes_snapshot.id_loja FROM `links_boadica_detalhes_snapshot`,`lojas_boadica`,`links_boadica` WHERE links_boadica_detalhes_snapshot.id_produto=links_boadica.id AND links_boadica_detalhes_snapshot.id_loja=lojas_boadica.id_loja AND links_boadica_detalhes_snapshot.id_produto=$id_produto ORDER BY links_boadica_detalhes_snapshot.preco";
   ...
   $flag_ocultar='0';
    ...
    //echo "$flag_ocultar / $id_produto / $id_loja<br>";
      ...
      IF((($nosso_preco>$preco) AND ($nosso_preco_cb2>$preco)) AND ($flag_predio==1) AND ($nosso_preco<>9999 OR $nosso_preco_cb2<>9999) AND $flag_ocultar=="0"){
              ...
              IF ($data<$dthoje_eua){
                   $img_dia="<img src='../imagens/moon.png' width='32' height='32'>";
              }
              ELSE {
                   $img_dia="<img src='../imagens/sun.png' width='32' height='32'>";
              }
