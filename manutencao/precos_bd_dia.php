...
IF($modo=='hora'){
  ECHO "Para ordenar por Produto, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=produto&atualizavel=".$atualizavel."&data=".$data."&only_actives=".$only_actives."&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";
  $texto_modo="Listado por hora, ";
  $clausula_query_order_by="ORDER BY links_boadica_detalhes_lojas.data DESC";
}
  ELSE{
    ECHO "Para ordenar por Hora, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=hora&atualizavel=".$atualizavel."&data=".$data."&only_actives=".$only_actives."&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";    
...
IF($atualizavel=='0'){
  ECHO "Para exibir somente atualizaveis, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=1&data=".$data."&only_actives=".$only_actives."&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";
  $texto_atualizavel="todos os produtos atualizaveis ou nao no dia, ";
}
  ELSE {
    ECHO "Para exibir todos (atualizaveis ou nao), clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=0&data=".$data."&only_actives=".$only_actives."&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";
  $texto_atualizavel="somente produtos atualizaveis no dia, ";
  }
  
IF($only_actives=='0'){
  ECHO "Para exibir somente Ativos no BD, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=1&data=".$data."&only_actives=1&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";
  $texto_ativo_bd="produtos ativos ou nao no Boa Dica";
}
  ELSE {
    ECHO "Para exibir todos (ativos ou nao no BD), clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=".$atualizavel."&data=".$data."&only_actives=0&filtro_loja=".$filtro_loja."&only_losing=".$only_losing."'>Aqui</a><BR>";
    $texto_ativo_bd="somente produtos ativos no Boa Dica";  }
  
IF ($filtro_loja<>""){
  ECHO "Para voltar a exibir todas as lojas, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=".$atualizavel."&data=".$data."&only_actives=".$only_actives."&filtro_loja=&only_losing=".$only_losing."'>Aqui</a><BR>";
...
IF($only_losing==1){
  ECHO "Para exibir todos os itens que estamos perdendo ou empatando para concorrencia, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=".$atualizavel."&data=".$data."&only_actives=$only_actives&filtro_loja=".$filtro_loja."&only_losing=0'>Aqui</a><BR>";  
  $texto_only_losing=", somente produtos que estamos acima da concorrencia";}
ELSE {
  ECHO "Para exibir somente itens que estamos perdendo para concorrencia, clique <a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=".$atualizavel."&data=".$data."&only_actives=$only_actives&filtro_loja=".$filtro_loja."&only_losing=1'>Aqui</a><BR>";
...
echo "<table>";
echo "<tr><td>Hora</td><td>&nbsp</td><td>&nbsp</td><td>Loja</td><td></td><td witdh='5'>Id</td><td>Marca</td><td>Produto</td><td>&nbsp</td><td>Preço Cabos</td><td>Concorrencia</td><td colspan='3' align='center' title='Ativo BD, Status vitoria, Editavel'>Flags</td><td align='right'>Link BD</td><td align='right'>Link Grupo</td><td align='right'>Link ID</td></tr>";
...
$preco_cabos=mysql_result($resultado2,0,0);
    ...
    IF ($atualizavel=='0' AND $flag_ocultar=='0'){ // flag para exibir todos os itens, seja possivel ou nao sua atualizacao no dia
      echo "<tr><td>".$hora."</td><td><a href='http://www.cabos.etc.br/manutencao/precos_bd_dia.php?modo=".$modo."&atualizavel=1&data=".$data."&only_actives=".$only_actives."&filtro_loja=".trim($loja)."&only_losing=".$only_losing."'><img src='$imagem_cadeado'></a></td><td><DIV><FONT COLOR='".$cor."'>".substr($loja,0,25)."</FONT></DIV></td><td><a target='_blank' href='precos_bd_rotinas.php?modo=ocultar_link&idproduto=$id_geral_produto&idloja=$id_loja'><img src='../imagens/on.png'></a></td><td>".$id_geral_produto."</td><td>".$marca."</td><td>".$produto."</td><td align='right'>".$preco_cabos."</td><td><FONT COLOR='$cor_fonte_preco_concorrencia'>".$preco."</FONT></td><td><IMG SRC='../imagens/".$imagem_power."'></td><td><IMG SRC='../imagens/".$imagem_bola."'></td><td><IMG SRC='../imagens/".$tipo_bolinha."'></td><td><a href='".$link."'  TARGET='_blank'>Link BD</a></td><td><a href='../manutencao/precos_bd.php?cdproduto=".$cd_produto."'  TARGET='_blank'>".$cd_produto." (".$quantidade_anuncios.")</a></td><td><a href='../manutencao/precos_bd.php?modo=pendentes&id_pesquisa=".$id_produto."'  TARGET='_blank'>".'Link '.$id_produto." </a></td></tr>";
