  ...
  <body>
    ...
    <?
      ...
      */
      ...
      $query="SELECT links_boadica.id, links_boadica.cdproduto 
              FROM `links_boadica` 
      ...
      IF($horasistema==12){
        $query_zerar_ocultar_meiodia="DELETE FROM `links_boadica_ocultar` 
                                      WHERE `flagmeiodia`=1";
      ...
      // Query para buscar os ids, mas ordenados pelo cdproduto e depois pelo id do anuncio
      ...
      while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
        $id_produto=$row[0]; 
        $query_nosso_preco="SELECT links_boadica_detalhes_snapshot.preco 
                            FROM links_boadica_detalhes_snapshot 
                            WHERE id_loja=$cdloja1_linksboadica AND id_produto='$id_produto'";
        //echo "$query_nosso_preco<br>";
        ...
        while ($row_detalhes = mysql_fetch_array($resultado_detalhes, MYSQL_NUM)) {
          ...
          IF((($nosso_preco>$preco) AND ($nosso_preco_cb2>$preco)) AND ($flag_predio==1) AND ($nosso_preco<>9999 OR $nosso_preco_cb2<>9999) AND $flag_ocultar=="0"){
            ...
            IF ($modo=="grupo"){
              $lixeiraamarela="&nbsp";
              $identificador_produto="<a href='../t/BDJavascript.php?cdproduto=$cdproduto' target='_blank'>$cdproduto</a>";              
            }
            ELSE { 
            ...
            IF($flag_ocultar=="0"){
              $conteudoDiv="<div id='div$cdproduto' name='nome$cdproduto' class='mostrar'>
                              ...
                              <div class='direita32'><a href='elisthistorico.php?cdproduto=".$cdproduto."'  TARGET='_blank'><img src='../imagens/lista.png' title='Histórico de custos\ndo produto'></a></div>
          ...
          //}
        } // Final do loop iniciado em 268
            
      }  // Final do loop iniciado em 223
