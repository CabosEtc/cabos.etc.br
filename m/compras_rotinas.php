...
<body>
	<?
		...
		echo "Query pesquisa codigo do estoque: $query<br>";

		if ($modo=="incluir"){
			// seleciona o produto no banco de dados
			$query="INSERT INTO compras(idcompra, pedido, cdloja, cdstatus, dtcompra, dtchegada, moeda, cdtransportadora, endereco, cdrastreamento, cotacao_us, idpaypal, cartao, observacao) 
					VALUES ('$iditem', '$pedido', '$cdestoque', '$cdstatus', '$dtcompra', '$dtchegada', '$moeda', '$cdtransportadora', '$endereco', '$cdrastreamento', '$cotacao_us', '$idpaypal', '$cartao', '$observacao')";
			$resultado = mysql_query($query,$conexao);
			echo $query;
			...
			echo "</table>";
			echo "<br><a href='cinc.php'>Voltar p/ incluir outro produto</a>";
			echo "<br><br>";
			echo "<a href='../manutencao/compras_editar.php?iditem=".$idinclusao."'>Corrigir erros na entrada dos dados</a>";
			echo "<br><br>";
			
			$sub_query="SELECT COUNT(*) AS total 
						FROM fornecedores 
						WHERE paypal='".$idpaypal."'";
				//  echo $sub_query2;
