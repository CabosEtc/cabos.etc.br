<html>
  ...
  <body>
    ...
    <?
      ...
      //echo "<div>O código da loja é: $cdLoja</div>";
      ...
      if($cdloja==1 OR $cdloja==8 OR $cdloja==9){ // Cabos 329 ou Cabos 318 ou Cabos 212
        $idLoja1=19; // Cabos
        $idLoja2=451; // Cabos 2
      ...
      if($cdloja==4 OR $cdloja==10){ // Super Game ou Supernova
        $idLoja1=2; // Super Game
        $idLoja2=239; // Supernova
        $idLoja3=743; // Super Games 2
        $idLoja4=732; // Supernova 2
        $abreviacaoLoja1="SG";
        $abreviacaoLoja2="SN";
        $abreviacaoLoja3="SG2";
        $abreviacaoLoja4="SN2";
      ...
      $queryContaPublicados = "SELECT count(*) as total FROM bd_mysnapshot WHERE publicado=1 and idloja=$idLoja3";
      $resultadoContaPublicados=mysql_query($queryContaPublicados, $conexao);
      $totalPublicadoLoja3 = mysql_result($resultadoContaPublicados,0,0);
      //echo "<div>Total Publicado $abreviacao2Loja2 : <span id=\"totalpublicadoloja2\">$totalPublicadoLoja2</span></div>";
      ...
      echo  "<div class='info-bd'>
                <div class='row'>
                  <div class='col-9'>
                    ...
                    <div class='row'>
                    <div id='divTotalPublicado2' class='col info-bd-totais'>
                      $abreviacaoLoja3: <span id=\"totalpublicadoloja2\">$totalPublicadoLoja3</span>
                    </div>
                  </div>
                  </div>
                  <div class='col-3'>
      ...
      ECHO "<div class='itemMenu'><input type='checkbox' id='servidorLocal' name='servidorLocal' data-id='' value='1'>Servidor local</div>";  
      ...
      echo "<thead>
              <tr>
                ...
                <th colspan='2'>$abreviacaoLoja2</th>
                <th colspan='5'>$abreviacaoLoja3</th>
              </tr>
            </thead>
      ...
      while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
        ...
        // Loja 329
        $queryPrecoCabos="SELECT preco 
                          FROM links_boadica_detalhes_snapshot 
                          WHERE id_produto=$idProduto AND id_loja=$idLoja1";
        ...
        $queryPrecoCabos2=" SELECT preco 
                            FROM links_boadica_detalhes_snapshot 
                            WHERE id_produto=$idProduto AND id_loja=$idLoja2";
        ...
        // Loja 318
        $queryPrecoCabos3=" SELECT preco 
                            FROM links_boadica_detalhes_snapshot 
                            WHERE id_produto=$idProduto AND id_loja=$idLoja3";
        $resultadoPrecoCabos3=mysql_query($queryPrecoCabos3, $conexao);
        $precoCabos3=number_format(mysql_result($resultadoPrecoCabos3,0,0),2,".","");
        //echo "<td>Preço Cabos 2 $precoCabos2</td></tr>";
          
        if(($precoProduto<$precoCabos OR $precoCabos==0.00) AND ($precoProduto<$precoCabos2 OR $precoCabos2==0.00) AND ($precoProduto<$precoCabos3 OR $precoCabos3==0.00)){
          $fontColor="<font color=\"#FF0000\">";
          $flagPerdendo=1;
        }
        else{
        ...
        if ($precoCabos3==0.00){
          $flagCabos3Desativado=1;
          // Adicionado em 24Out20 para mostrar o preço que está no site do boadica, mesmo desativado
          $queryPrecoFantasma="SELECT bd_mysnapshot.valor, bd_mysnapshot.alterado, bd_mysnapshot.publicado 
                              from bd_mysnapshot, bd_id_anuncios where bd_mysnapshot.idanunciobd=bd_id_anuncios.idanunciobd 
                              AND bd_id_anuncios.idlinkbd=$idProduto AND bd_mysnapshot.idloja=$idLoja3";
          $resultadoPrecoFantasma=mysql_query($queryPrecoFantasma, $conexao);
          IF($resultadoPrecoFantasma){
            $precoCabos3=mysql_result($resultadoPrecoFantasma,0,0);
            // Faz o preço piscar
            IF ($precoCabos3==$precoProduto){
              $blinkCb3='class="blink"';
            }
            $alteradoBDCabos3=mysql_result($resultadoPrecoFantasma,0,1);
            IF($alteradoBDCabos3==0){$alteravelBDCabos2="Sim";$corFontePrecoBDCabos3="#90EE90";} ELSE {$alteravelBDCabos3="Não";$corFontePrecoBDCabos3="#FFB6C1";}
            $precoCabos3="<span title=\"Desativado no BD\nAlterável no BD: $alteravelBDCabos3\"><font color=\"$corFontePrecoBDCabos3\">$precoCabos3</font></span>";  
          }
            ELSE{
              $precoCabos3="----";
            }
        ...
        if(($precoProdutoMaisUmCentavo==$precoCabos) OR ($precoProdutoMaisUmCentavo==$precoCabos2) OR ($precoProdutoMaisUmCentavo==$precoCabos3)){
          $flagCentavo=1;
        ...
        if(($idLoja== "2") or ($idLoja== "239") or ($idLoja=="743")  or ($idLoja=="732") or ($idLoja=="19") or ($idLoja=="451") or ($idLoja=="784")){ // lojas parceiras
        ...
        if ((($flagSoPerdendo==1 AND $flagPerdendo==1) OR ($flagSoPerdendo<>1)) AND (!$flagCabosDesativado OR !$flagCabos2Desativado OR !$flagCabos3Desativado))  {
          ...
          $query_ultima_alteracao=" SELECT data 
                                    FROM links_boadica_detalhes_lojas 
                                    WHERE id_loja='$idLoja1' AND id_produto='$idProduto' 
                                    ORDER BY data DESC";
          ...
          // Pesquisa ultima alteração de preços [ Cabos 318 ]
          $query_ultima_alteracao=" SELECT data 
                                    FROM links_boadica_detalhes_lojas 
                                    WHERE id_loja='$idLoja3' AND id_produto='$idProduto' 
                                    ORDER BY data DESC";
          //echo "Cabos> $query_ultima_alteracao<br>";
          $resultado_ultima_alteracao = mysql_query($query_ultima_alteracao,$conexao);
          ...
          IF ($data_ultima_alteracao==$dthoje_eua){  
            $imgAlteradoHojeCb3="<img src='../imagens/fire.png'  width='16' height='16' >";
            $flagAlteradoHojeCb3=1;
          }
          ELSE {
              $imgAlteradoHojeCb3="<img src='../imagens/leftarrow.gif'>";
              $flagAlteradoHojeCb3=0;   
          }
          ...
          AND idloja IN ($idLoja1 , $idLoja2 , $idLoja3)";
          ...
          if(($flagAlteradoHojeCb<>1 OR $flagAlteradoHojeCb2<>1 OR $flagAlteradoHojeCb3<>1) AND $flagPodeExibir){
            ...
            echo "<tr class='$classeTr'>
                    ...
                    <td width='20'><a href='BDJs.php?cdproduto=$codigoProduto' target='_blank'><img src='../imagens/thunder.png' width='16' height='16' title='Usar servidores externos'/></a></td>
                    <td width='20'><a href='BDProduto.php?cdproduto=$codigoProduto' target='_blank'><img src='../imagens/toDo.png' width='16' height='16' title='Usar servidor local'/></a></td>
                    ...
                    <td $blinkCb3><span id='precoId$idProduto"."Loja2'>$precoCabos3</span></td>
                    <td>$imgAlteradoHojeCb3</td>
                  ...
                  <tr style='display: none;' id='divLinha$contadorLinhasExibidas'>
                    <td colspan='22'>Rotinas de automatização
                    ...
                    <img src='..\imagens\btnAlteracoesBDCinzaCinzaVerde.png' onclick='alert(\"Nada por enquanto...\");'  style='padding: 0px 10px 0px 10px;' />
                  ...
                  <tr style='display: none;' id='trLinhaComandoEfetuado$contadorLinhasExibidas' class='alert alert-success'>
                    <td colspan='22' id='tdLinhaComandoEfetuado$contadorLinhasExibidas'>
                      teste de informacao
                    </td>
                  </tr>

                  <tr style='display: none;' id='tr$contadorLinhasExibidas'>
                    <td colspan='22'><div id='divPrecosBD$contadorLinhasExibidas'>&nbsp</div></td>
                  </tr>";
      ...
      } // Fim do while - da linha de exibicao do produto (linha 214)
      ...
      while ($row = mysql_fetch_array($resultadoSpy, MYSQL_NUM)) {
        ...
        echo "<tr style='background-color: #B0E0E6;'>
                ...
                <td align='center'>
                  <a href='BDProduto.php?cdproduto=$cdProduto' target='_blank'>
                    <img src='../imagens/toDo.png' width='16' heigth='16' />
                  </a>
                </td>
                <td width='95'>$marcaProduto</td>
