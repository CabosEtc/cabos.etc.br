<html>
  ...
  <body>
    ...
    <script src="js/BDPrecos.js"><!-- Copia conteudo para area de transferencia--></script>
    ...
    <?
      ...
      $showitens=$_REQUEST["showitens"];
      IF($showitens==""){
        $clausula_showitens="1=1";
      }
      ELSE{
        $clausula_showitens="id IN (".$showitens.")";
      }

      // Seta os ids das Lojas para pesquisa do preço no snapshot
      if($cdloja==1){ // Cabos
        $idLoja1=19; // Cabos 329
        $idLoja2=451; // Cabos 212
        $idLoja3=784; // Cabos 318
        $abreviacaoLoja1="Cb1";
        $abreviacaoLoja2="Cb2";
        $abreviacaoLoja3="Cb3";
        $abreviacao2Loja1="Cabos";
        $abreviacao2Loja2="Cabos 2";
        $abreviacao2Loja3="Cabos 318";
      }

      if($cdloja==4){ // Super Game
        $idLoja1=2; // Super Game
        $idLoja2=239; // Supernova
        $idLoja3=743; // Super Games 2
        $idLoja3=732; // Supernova 2
        $abreviacaoLoja1="SG";
        $abreviacaoLoja2="SN";
        $abreviacaoLoja3="SG2";
        $abreviacaoLoja4="SN2";
        ...
        $abreviacao2Loja3="Supe Games 2";
        $abreviacao2Loja4="Supernova 2";
      }
      ...
      IF ($modo=='pendentes'){
        ...
        ELSE {
          ...
          */      	
          $query="SELECT id, produto, link, cdproduto, flag_ativo_boadica, marca, links_boadica.localizador, links_boadica.box 
                  FROM links_boadica 
                  WHERE cdproduto='$cdproduto' AND $clausula_showitens AND $clausula_ativo 
                  ORDER BY id";
          ...
          ELSE { 
            ...
            ELSE {// usado na pesquisa por um unico id (2 mais usado)
              $query="SELECT id, produto, link, cdproduto, flag_ativo_boadica, marca, links_boadica.localizador, links_boadica.box 
                      FROM links_boadica 
                      WHERE (id>='$inicio_id' AND id<='".($inicio_id+9)."') AND flag_ativo IN (0,1,2) 
      ...
      if($inicio_id<>""){
        $queryCdProdutoById="SELECT cdproduto from links_boadica WHERE id='$inicio_id'";
        $resultadoCdProdutoById=mysql_query($queryCdProdutoById, $conexao);
        $cdProdutoById=mysql_result($resultadoCdProdutoById,0,0); // só para os links abaixo funcionarem...
      ...
      echo "<h3 title='$dthoje_bra - $hora'>Relatório de precos do Boadica: $cdproduto ($nomeProdutoTabelaProdutos) - $resultado_quant produtos listados</h3>";
      echo "<div style='margin: 0 0 10px 0'>
            <a href='index.php' ><img src='../imagens/home.png' /></a>
            <a href='elisthistorico.php?cdproduto=$cdProdutoById' target='_blank'><img src='../imagens/lista.png' /></a>
            <a href='BDPrecos.php?cdproduto=$cdProdutoById&showall=1' target='_blank'><img src='../imagens/spy.png' /></a>";
            
              echo "<a href='BDPrecosRotinas.php?modo=ocultar_cdproduto&cdproduto=$cdProdutoById' target='_blank'>
              <img src='../imagens/trashred.png' title='Esconde o anúncio de todas as lojas até o fim do dia (CUIDADO!)'/></a>";
        ...
        ELSE
            {
              ...
              // Verifica se o codigo consta da tabela bd_id_anuncios
              $queryVerificaIdAnunciosBD="SELECT idloja from bd_id_anuncios WHERE idlinkbd='$id' 
              ...
              if($verificaResultado>0) {
                ...
                if ($verificaResultado==2){
                  $atom="<img src='../imagens/atom.png' id='atom$contadorLinhasExibidas' title='teste' onclick='document.getElementById(\"divLinha$contadorLinhasExibidas\").style.display=\"table-row\"' />";
                }
              }
              else {
                $flagIdAnuncioBD=0;
        ...
        $separador=" - ";
          ...
          <a target='_blank' href='BDPrecosRotinas.php?modo=ocultar_link&idproduto=$id&cdloja=$cdloja&idloja=0'><img src='../imagens/trashred.png' title='Esconde o anuncio de todas as lojas até o fim do dia (CUIDADO!)'></a>";     
            ...
            echo "<tr>
                    ...
                    <td>".$div_produto.$separador.$div_localizador."</td>
                    <td>[".$marca."]</td>
                    <td>$box</td>
                    <td>[$id]
                    </td>
                  ...
                  <tr>
                    <td colspan='8'>
                    <span class='control-copydiv' onClick=\"fieldtoclipboard.copyfield(event, 'select".$contador_item."'); marcaiconecopy('copy$id'); \">
                      <img id='copy".$id."' src='../imagens/copy.png' title='Copia para área de transferência'>
                    </span>
                    <a href='".$link."' TARGET='_blank'>
                      <img src='../imagens/coruja.png' title='Exibe página do Boa Dica'>
                    </a>
                    <a href='BDPrecos.php?inicio_id=$id&limite=1' $flag_target_blank>
                      <img id='$idcamera' src='../imagens/camera.png' onclick='trocacamera(\"$idcamera\");' title='Abre este único ID'>
                    </a>
                    <a href='../t/BDJavascript.php?cdproduto=$cdProdutoById' target='_blank'>
                      <img src='../imagens/target.png' width='32' height='32' />
                    </a>
                    $clock_and_trashred
                    $atom
                    </td>
          ...
          <input type='text' size='10' maxlength='6' id='novoValor$contadorLinhasExibidas' value='$precoSugeridoProduto'>
          <img src='..\imagens\btnAlteracoesBDVerdeVermelho.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, 1, $idLoja2, 0, \"atom$contadorLinhasExibidas\");' title='Vou ativar na $abreviacao2Loja1\n desativar na $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />
          
          <img src='..\imagens\btnAlteracoesBDVerdeCinza.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, 1, $idLoja2, \"X\", \"atom$contadorLinhasExibidas\");' title='Vou ativar na $abreviacao2Loja1\n Não vou alterar a $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />



          <img src='..\imagens\btnAlteracoesBDVermelhoVerde.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, 0, $idLoja2, 1, \"atom$contadorLinhasExibidas\");' title='Vou desativar na $abreviacao2Loja1\n ativar na $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />

          <img src='..\imagens\btnAlteracoesBDCinzaVerde.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, \"X\", $idLoja2, 1, \"atom$contadorLinhasExibidas\");' title='Não vou alterar a $abreviacao2Loja1\n Vou ativar na $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />



          <img src='..\imagens\btnAlteracoesBDVerdeVerde.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, 1, $idLoja2, 1, \"atom$contadorLinhasExibidas\"); 'title='Vou ativar nas lojas $abreviacao2Loja1 e $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />
          <img src='..\imagens\btnAlteracoesBDVermelhoVermelho.png' onclick='ajustarPreco(\"$id\", \"novoValor$contadorLinhasExibidas\", $idLoja1, 0, $idLoja2, 0, \"atom$contadorLinhasExibidas\");' title='Vou desativar nas lojas $abreviacao2Loja1 e $abreviacao2Loja2' style='padding: 0px 10px 0px 10px;' />
          </td></tr>";
        ...
        */
        ...
        foreach ($linhas_dados as $field) {
          ...
          if(strpos($field,"target") AND
            ...
            VALUES ('null', CURRENT_TIMESTAMP, '300', '1', '$id', '$nome_loja','BDPrecos')";
        ...
        $hidden='&nbsp';
          ...
          ECHO "<TD>$quant_lojas_predio</TD>
                <TD>$hidden</TD>
                <td>
                  <a target='_blank' href='BDPrecosRotinas.php?modo=ocultar_link&idproduto=$id&cdloja=$cdloja&idloja=$id_loja_todas'>
                    <img src='../imagens/trash.png' width='16' height='16' title='Esconde o anuncio da loja $id_loja_todas'>
                  </a>
                </td>
                <td>
                  <a target='_blank' href='BDPrecosRotinas.php?modo=ocultar_link&idproduto=$id&cdloja=$cdloja&idloja=$id_loja_todas&preco=0&flagmeiodia=1'>
                    <img src='../imagens/clock.png' width='16' height='16' title='Esconde o anuncio da loja $id_loja_todas ate as 12:00h'>
                  </a>
                </td>
              </TR>"; // Fecha a linha de informacao de cada loja
          } 
          // fim do Nó 2 (preco)
