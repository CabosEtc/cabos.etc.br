...
<body>
	...
	include("mmenu.php");  
		...
		echo "<div>Cotação do Dolar: ".$cotacao_dolar."</div>";
		echo "<div>Centro de Custos: $centroDeCustos</div>";
					



	IF($modo=="todas_as_lojas"){
	...
	//	$valor_total_compras=0;
		...
		while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
			...
			$query4="	SELECT SUM(notas_detalhes.quantidade*notas_detalhes.vlproduto) as vlvendatotaldoproduto 
						FROM notas_detalhes, notas 
						WHERE cdproduto='".$cdproduto."' and notas.idnota=notas_detalhes.idnota 
						AND MONTH(notas.dtnota)='".$mes."' AND YEAR(notas.dtnota)='".$ano."' 
						AND notas.cdloja = '".$cdloja."' ";
			//echo $query4;
			...
			//echo "CdProduto: $cdproduto | Dtmovimento $dtmovimento<br>";
