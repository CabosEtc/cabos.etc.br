<html>
  <!--  Historico de alterações
        15Dez21 Diretorio alterado para /m, incluida rotina para verificar o dia e hora em que a rotina será executada, isto foi feito por causa da limitação do
        Cron Job na Hostinger.
        19Dez21 CURRENT_TIMESTAMP foi substituido pela variavel $timestampSaoPaulo por causa da troca do servidor para Hostinger
        16Jun22 Rotinas adicionadas por conta da inclusão da loja 318 no sistema, tambem ajustes para Super game, Super games 2, Supernova e Supernova 2.
  -->
  <head>
  ...
  <body onload='timedCount()'>
    ...
    ***********************************************/
    ...
    <?php 
      ...
      //Verifica se rotina está habilitada

      $queryVerificaStatusAutoPDB=" SELECT autopbd 
                                    FROM parametros 
                                    WHERE cdloja=1"; // Alterado em 16Jun22, quem define será somente a loja Cabos 329.
      ...
      $diaSemanaNumero = date('w', time()); // Retorna 0-6 Domingo-Sabado
      //echo "Dia da semana: $diaSemanaNumero<br>Hora: $hora<br>";

      if($statusAutoPDB==false){
        exit("Rotina Interrompida");
      }
      ...
      switch ($diaSemanaNumero) {
        ...
        case 3:
        case 4:
        case 5: // Mesma ação para dias 1-5 (seg-sex)
          if(($hora>"00:00:00" and $hora<"04:00:00") or ($hora>"08:00:00" and $hora<"23:59:59")){
            echo "Rodando<br>";
          }
          else{
            exit("Fora do horario");
        ...
        case 6: //Sabado
          if(($hora>"00:00:00" and $hora<"04:00:00") or ($hora>"06:00:00" and $hora<"12:59:59") or ($hora>"18:00:00" and $hora<"20:59:59")){
            echo "Rodando<br>";
          }
          else{
            exit("Fora do horario");
      ...
      $query_contaID="SELECT id 
                      from links_boadica 
                      WHERE flag_ativo IN (0,1,2) or flag_ativosg IN (0,1,2) 
                      ORDER BY id DESC";
      ...
      IF($inicio_id>=$ultimo_id){
        $inicio_id=1; // Volta para o inicio se atingir o ultimo
      }
      ...
      $query="SELECT id, produto, link, cdproduto, flag_ativo_boadica, marca, links_boadica.localizador 
              FROM links_boadica 
              WHERE id>='$inicio_id' AND flag_ativo IN (0,1,2)  
              ORDER BY id LIMIT 8";
      ...
      while ($row = mysql_fetch_array($resultado, MYSQL_NUM)) {
        ...
        // Nova rotina para pesquisar a Cabos 318  (id 784)
        $findme="Cabos e Etc 2";
        $flag=strpos($dados_novos,$findme);
        IF ($flag === false) { // Se não achar nosso anuncio
          //echo "<font color='red'>**Anuncio Desativado!!!!!**</font><br>";
          IF ($flag === false AND $tamanho_pagina<>133){ // Se não achar nosso anuncio e a pagina tiver retornado algum resultado valido
            $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdcabos318='0' WHERE id ='$id'";
            $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
            $imagem_power_cb318="<img src='../imagens/power_off.gif'>";
            // echo $query_flag_ativo;
          }
        } 			
        
        ELSE { // Se achar nosso anuncio
          $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdcabos318='1' WHERE id ='$id'";
          $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
          $imagem_power_cb318="<img src='../imagens/power_on.gif'>";
          // echo $query_flag_ativo;
        }
        ...
        // Nova rotina para pesquisar a Super Games 2 (id 743)
        $findme_sg2="Super Games 2";
        ...
        $findme_sn="Supernova";
        $flag_sn=strpos($dados_novos,$findme_sn);
        
        IF ($flag_sn === false AND $tamanho_pagina<>133){ // Se não achar nosso anuncio e a pagina tiver retornado algum resultado valido
            $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdsn='0' WHERE id ='$id'";
            $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
            $imagem_power_sn="<img src='../imagens/power_off.gif'>";
            // echo $query_flag_ativo;
            //echo "Desativado na cabos 2";
        }
              
        ELSE{ // Se achar o anuncio
          $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdsn='1' WHERE id ='$id'";
          $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
          $imagem_power_sn="<img src='../imagens/power_on.gif'>";
            // echo $query_flag_ativo;
          //echo "Ativo na cabos 2";
        }
        ...
        $findme_sn2="Supernova 2";
        $flag_sn2=strpos($dados_novos,$findme_sn2);
        
        IF ($flag_sn2 === false AND $tamanho_pagina<>133){ // Se não achar nosso anuncio e a pagina tiver retornado algum resultado valido
            $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdsn2='0' WHERE id ='$id'";
            $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
            $imagem_power_sn2="<img src='../imagens/power_off.gif'>";
            // echo $query_flag_ativo;
            //echo "Desativado na cabos 2";
        }
              
        ELSE{ // Se achar o anuncio 
          $query_flag_ativo_boadica="UPDATE links_boadica SET flag_ativo_bdsn2='1' WHERE id ='$id'";
          $resultado_flag_ativo_boadica = mysql_query($query_flag_ativo_boadica,$conexao);
          $imagem_power_sn2="<img src='../imagens/power_on.gif'>";
            // echo $query_flag_ativo;
          //echo "Ativo na cabos 2";
        }
          ...
          $query_ultima_alteracao=" SELECT data from links_boadica_detalhes_lojas 
                                    WHERE id_loja='19' AND id_produto='$id' 
                                    ORDER BY data DESC";
          ...
          $query_ultima_alteracao=" SELECT data from links_boadica_detalhes_lojas 
                                    WHERE id_loja='784' AND id_produto='$id' 
                                    ORDER BY data DESC";
          //echo $query_ultima_alteracao;
          $resultado_ultima_alteracao = mysql_query($query_ultima_alteracao,$conexao);
          ...
          IF ($data_ultima_alteracao==$dthoje_eua){  
            $flag_ativo_cb318="<img src='../imagens/check.gif'>";
          }
          ELSE {
            $flag_ativo_cb318="<img src='../imagens/leftarrow.gif'>";   
          }
      ...
      $query_verifca_links_referencia="SELECT COUNT(flag_ativo) AS total FROM links_boadica WHERE cdproduto=$cdproduto AND flag_ativo='2'";
          ...
          $link_spy="$nbsp";
            ...
            echo "<table>
                    <tr>
                      ...
                      <td>$imagem_power_cb318</td>
                      <td>$flag_ativo_cb318</td>
        ...
        foreach ($linhas_dados as $field) {		
          ...
          if(strpos($field,"target") AND
            ...
            $query_id=" SELECT id_loja, flag_predio 
                        FROM lojas_boadica 
                        WHERE nome LIKE '%".$nome_loja."%'";
            ...
            if(mysql_num_rows($resultado_id)>0){ // se a loja é cadastrada
              ...
              $query_evitar_duplicidades="DELETE FROM log 
                                          WHERE codigo=300 AND inf2='$nome_loja'";
              //echo $query_apaga_dados_anteriores;
              $resultado_evitar_duplicidades = mysql_query($query_evitar_duplicidades,$conexao);
              $query_inserir_log="INSERT INTO log(`idlog`, `data`,  `codigo`, `loja`, `inf1`, `inf2`, `inf3`) 
                                  VALUES ('null', '$timestampSaoPaulo', '300', '1', '$id', '$nome_loja', 'AutoPBD')";
          ...
          if(strpos($field,"R$") AND 
            ...
            if(($id_loja==19 or $id_loja==451 or $id_loja==784) and $quant_lojas_predio==1){
            ...
            if($precoNossasLojasEmPrimeiro<>"" and $precoNossasLojasEmPrimeiro<>0 and $precoSegundoLugar<>0 and ($precoNossasLojasEmPrimeiro<($precoSegundoLugar-0.01))){
              $query_inserir_log="INSERT INTO log(`idlog`, `data`,  `codigo`, `loja`, `inf1`, `inf2`, `inf3`, `inf4`) 
                                  VALUES ('null', '$timestampSaoPaulo', '302', '1', '$id', '$precoNossasLojasEmPrimeiro', '$precoSegundoLugar', '$nome_loja')";
              //$resultado_inserir_log = mysql_query($query_inserir_log,$conexao);	
            }
            ...
            // Insere no banco de dados, somente se a loja for cadastrada e o preço for diferente da pesquisa anterior	
              $query_pesquisa_preco_antigo="SELECT preco, data 
                                            ...
                                            WHERE id_loja='$id_loja' AND id_produto='$id' 
                                            ORDER BY data DESC";
              ...
              IF (($quantidade_itens==0) OR ($preco_anterior<>$preco)){		
                ...
                $query_precos="INSERT INTO links_boadica_detalhes_lojas (`id`, `id_loja`, `data`, `id_produto`, `preco`, `origem`) 
                                VALUES (NULL, $id_loja, '$timestampSaoPaulo', $id, $preco, 'AutoPBD');";
            ...
            // Rotina adicionada em 28Maio2018 para permitir guardar um snapshot de todos os precos de determinado produto na data corrente
                $query_precos_snapshot="INSERT INTO links_boadica_detalhes_snapshot (`id`, `id_loja`, `data`, `id_produto`, `preco`, `origem`) 
                                        VALUES (NULL, $id_loja, '$timestampSaoPaulo', $id, $preco, 'AutoPBD');";
        ...
        $query_inserir_log="INSERT INTO log(`idlog`, `data`,  `codigo`, `loja`, `inf1`, `inf2`) 
                            VALUES ('null', '$timestampSaoPaulo', '200', '1', '$id', 'AutoPBD')";
        $resultado_inserir_log = mysql_query($query_inserir_log,$conexao);	
        ...
        IF($flag_erro_abertura<>"1"){
          ...
          ELSE
          {
            echo "<br>Houve erro na abertura de pelo menos uma das paginas, impossivel mostrar o relatorio consolidado<br>";
                  ...
                  ECHO "Ultimo id -> $id<br>";
                  $query_ultimo_id="UPDATE parametros 
                                    SET ultimo_link_bd='$id' 
                                    WHERE cdloja ='1'";
                  $resultado_ultimo_id = mysql_query($query_ultimo_id,$conexao);
    ?>
